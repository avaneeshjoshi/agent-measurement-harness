...
*Evidence:* ADR-0009 — prompt-grain agreement 53.1% (κ 0.41) against 81 human labels; segment-grain 38.5% (κ 0.24); segmenter reproduces 13/13 validatable calibration segments byte-exactly (validation report frozen at `data/evidence/adr-0009/validation_report.json`). Current output: 1,571 task_class records (0.1.0) across three unit grains. (ADR-0009 cites 1,446 — that was the count at its run date; the live file has since been regenerated over newer extractions, is the current figure, and lives at `~/.caliper/derived/classes/` per ADR-0012.)
